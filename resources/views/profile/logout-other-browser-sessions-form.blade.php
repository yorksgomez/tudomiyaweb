<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Administre y cierre sesión en otros dispositivos') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Si es necesario, usted puede cerrar sesión en cualquiera de los dispositivos aquí listados, se le recomienda también cambiar su contraseña') }}
        </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            <div class="text-sm text-gray-600">
                                {{ $session->agent->platform() ? $session->agent->platform() : 'Desconocida' }} - {{ $session->agent->browser() ? $session->agent->browser() : 'Desconocida' }}
                            </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Por favor ingrese su contraseña para poder continuar con la acción.') }}
